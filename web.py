...
from .database import init_db, save_case, get_cases, get_case_count, get_stats, get_erasure_cases
...
@app.route("/erasure")
def erasure():
    return render_template("erasure.html")


@app.route("/api/erasure")
def api_erasure():
    cases = get_erasure_cases()
    return jsonify({"ok": True, "cases": cases, "total": len(cases)})
